...
describe('Lottery Contract', () => {
  ...
  it('get test message', async() => {
    const msg = await lottery.methods.getTestMessage().call({from:accounts[0]});
    assert.equal("TestMessage",msg);
  });
  ...
  it('only manager can call pick winner', async () => {
    try{
      await lottery.eth.pickWinner().send({from:accounts[1]});
      assert(false);
    }
    catch(err){
      assert(err);
